...
def insert_reservoir_annotation_leader(color, digits):
    ...
    try:
        for i in range(0, len(globals.df_reservoirs)):
            ...
            globals.msp.add_text(head, height=globals.text_size, dxfattribs={'color': color, "style": "epa2HydChart"}).set_placement(
                (leader_up_end_x+6*globals.text_size, leader_up_end_y+2*globals.text_size), align=TextEntityAlignment.MIDDLE_RIGHT)
            globals.msp.add_text('ELEV', height=globals.text_size, dxfattribs={'color': color, "style": "epa2HydChart"}).set_placement(
                (leader_up_end_x+6*globals.text_size, leader_up_end_y+0.75*globals.text_size), align=TextEntityAlignment.MIDDLE_RIGHT)
            globals.msp.add_text('Pressure', height=globals.text_size, dxfattribs={'color': color, "style": "epa2HydChart"}).set_placement(
                (leader_up_end_x+6*globals.text_size, leader_up_end_y-0.75*globals.text_size), align=TextEntityAlignment.MIDDLE_RIGHT)
...
def insert_pump_annotation(color, digits):
    ...
    try:
        for i in range(0, len(globals.df_pumps)):
            ...
            globals.msp.add_text(f'Q:{Q_str}', height=globals.text_size, dxfattribs={'color': color, "style": "epa2HydChart"}).set_placement(
                (x+2*globals.text_size, y-offset[0]), align=TextEntityAlignment.MIDDLE_RIGHT)
            globals.msp.add_text(f'H:{H_str}', height=globals.text_size, dxfattribs={'color': color, "style": "epa2HydChart"}).set_placement(
                (x+2*globals.text_size, y-offset[1]), align=TextEntityAlignment.MIDDLE_RIGHT)
            msg = f'抽水機 {id} 已完成繪圖'
            log.renew_log(msg, False)
            log.set_log_to_button()
            progress_utils.set_progress_bar(forced_value=None)
...
def insert_valve_annotation(color):
    ...
    try:
        for i in range(0, len(globals.df_valves)):
            ...
            globals.msp.add_text(f'{Type}', height=globals.text_size, dxfattribs={
                                 'color': color, "style": "epa2HydChart"}).set_placement((x, y-offset[0]), align=TextEntityAlignment.MIDDLE_CENTER)
            globals.msp.add_text(f'{Setting}', height=globals.text_size, dxfattribs={
...
def insert_tank_annotation_leader(color, digits, width):
    ...
    try:
        for i in range(0, len(globals.df_tanks)):
            ...
            globals.msp.add_text(f'___T', height=globals.text_size, dxfattribs={'color': color, "style": "epa2HydChart"}).set_placement(
                (leader_up_end_x+10*globals.text_size, leader_up_end_y+3.25*globals.text_size), align=TextEntityAlignment.MIDDLE_RIGHT)
            globals.msp.add_text(f'Hwl:{max_elev}', height=globals.text_size, dxfattribs={'color': color, "style": "epa2HydChart"}).set_placement(
                (leader_up_end_x+10*globals.text_size, leader_up_end_y+2*globals.text_size), align=TextEntityAlignment.MIDDLE_RIGHT)
            globals.msp.add_text(f'Mwl:{min_elev}', height=globals.text_size, dxfattribs={'color': color, "style": "epa2HydChart"}).set_placement(
                (leader_up_end_x+10*globals.text_size, leader_up_end_y+0.75*globals.text_size), align=TextEntityAlignment.MIDDLE_RIGHT)
            globals.msp.add_text(f'Elev:{elev}', height=globals.text_size, dxfattribs={'color': color, "style": "epa2HydChart"}).set_placement(
                (leader_up_end_x+10*globals.text_size, leader_up_end_y-0.75*globals.text_size), align=TextEntityAlignment.MIDDLE_RIGHT)
            msg = f'水池 {id} 引線已完成繪圖'
            log.renew_log(msg, False)
            log.set_log_to_button()
