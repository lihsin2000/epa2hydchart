...
def process1(main_window_instance: 'MainWindow'):
    ...
    if inpFile and rptFile:
        dxfPath, _ = QFileDialog.getSaveFileName(
            main_window_instance, "儲存", "", filter='dxf (*.dxf)')
        file_name = os.path.basename(dxfPath)

        if dxfPath != '':
            config.df_Coords, config.df_Junctions, config.df_Reservoirs, config.df_Tanks, config.df_Pumps, config.df_Valves, config.df_Pipes, config.df_Vertices = main_window_instance.inp_to_df(
                inpFile, showtime=True)
            if config.hr_list == []:  # 單一時間結果
                try:
                    config.df_NodeResults = read_utils.readNodeResults(
                        hr=None, input=config.arranged_rpt_file_path)
                    config.df_LinkResults = read_utils.readLinkResults(
                        hr1=None, input=config.arranged_rpt_file_path)
                    matchLink, matchNode = main_window_instance.matchInpRptFile()
                    process2(main_window_instance, matchLink=matchLink,
                             matchNode=matchNode, dxfPath=dxfPath, hr='')
                ...
                for h in config.hr_list_select:
                    try:
                        config.df_NodeResults = read_utils.readNodeResults(
                            hr=h, input=config.arranged_rpt_file_path)
                        ...
                        if 1 <= i_hr2 <= len(config.hr_list) - 1:
                            ...
                            config.df_LinkResults = read_utils.readLinkResults(
                                hr1=h, hr2=hr2, input=config.arranged_rpt_file_path)
                        elif i_hr2 == len(config.hr_list):
                            config.df_LinkResults = read_utils.readLinkResults(
                                hr1=h, hr2='', input=config.arranged_rpt_file_path)
                        else:
                            main_window_instance.MainWindow.browser_log.append(
                                f'[Error]不明錯誤，中止匯出')
                            main_window_instance.MainWindow.browser_log.append(
                        ...
                        matchLink, matchNode = main_window_instance.matchInpRptFile()
                        process2(main_window_instance, matchLink=matchLink,
                                 matchNode=matchNode, dxfPath=dxfPath, hr=h)
...
def process2(main_window_instance: 'MainWindow', *args, **kwargs):
    ...
    if matchLink and matchNode:
        ...
        if dxfPath:
            ...
            main_window_instance.saveDXF(cad=config.cad, path=config.dxfPath)
            main_window_instance.MainWindow.browser_log.append(
                f'{dxfPathWithoutExtension}.dxf 已匯出')
            main_window_instance.setLogToButton()
            QCoreApplication.processEvents()

            svg_path = dxfPath.replace('.dxf', '.svg')
            main_window_instance.save_svg(
                msp=config.msp, cad=config.cad, path=svg_path)
            main_window_instance.MainWindow.browser_log.append(
                f'{dxfPathWithoutExtension}.svg 已匯出')
            main_window_instance.setLogToButton()
            QCoreApplication.processEvents()

            png_path = dxfPath.replace('.dxf', '.png')
            main_window_instance.convertSVGtoPNG(
                msp=config.msp, cad=config.cad, pngPath=png_path, svgPath=svg_path)
            main_window_instance.MainWindow.browser_log.append(
                f'{dxfPathWithoutExtension}.png 已匯出')
            main_window_instance.setLogToButton()
            QCoreApplication.processEvents()

            main_window_instance.MainWindow.browser_log.append('完成!')
            main_window_instance.MainWindow.browser_log.append(
                f'---------------------')
            main_window_instance.setLogToButton()
