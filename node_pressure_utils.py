...
def insert_pressure_annotation_leader(head_color, elev_color, width, auto_label_post, pipe_boundaries, draw_boundaries):
    """Insert pressure annotation leaders for all junctions with overlap detection."""
    ...
    try:
        for i in range(0, len(globals.df_junctions)):
            ...
            if auto_label_post:
                new_boundry = create_new_boundary(
                    start_x=start_x, start_y=start_y, align="RightTop", id=id)
                # Check overlap with both node boundaries and pipe annotations
                is_overlap = is_pressure_annotation_overlapping_any_boundary(
                    new_boundry=new_boundry, boundrys=boundrys, pipe_boundaries=pipe_boundaries, id=id, verbose=True)
                if is_overlap == False:
                    draw_pressure_annotation_leader(
                        parameters=parameters, align="RightTop")
                    draw_pressure_boundary(
                        boundry=new_boundry) if draw_boundaries else None
                else:
                    new_boundry = create_new_boundary(
                        start_x=start_x, start_y=start_y, align="LeftTop", id=id)
                    is_overlap = is_pressure_annotation_overlapping_any_boundary(
                        new_boundry=new_boundry, boundrys=boundrys, pipe_boundaries=pipe_boundaries, id=id, verbose=True)
                    if is_overlap == False:
                        draw_pressure_annotation_leader(
                            parameters=parameters, align="LeftTop")
                        draw_pressure_boundary(
                            boundry=new_boundry) if draw_boundaries else None
                    else:
                        new_boundry = create_new_boundary(
                            start_x=start_x, start_y=start_y, align="LeftBottom", id=id)
                        draw_pressure_annotation_leader(
                            parameters=parameters, align="LeftBottom")
                        draw_pressure_boundary(
                            boundry=new_boundry) if draw_boundaries else None
...
def draw_pressure_annotation_leader(parameters, align):
    """Draw a pressure annotation leader line with text labels."""
    from ezdxf.enums import TextEntityAlignment
    ...
    line_attribs = parameters['line_attribs']
    Head_attribs = parameters['Head_attribs']
